 ...
 * @since 2012-02-29
 */
public class NinePatcher extends TextureRegion {
	...
	public NinePatcher(final ITexture pTexture, final int pTexturePositionX, final int pTexturePositionY,
			final int pWidth, final int pHeight, final Rect pCenterPatch,  final Rect pField) {
		...
		mRegions[TOP_LEFT] = TextureRegionFactory.extractFromTexture(pTexture,x, y, pCenterPatch.left - x, pCenterPatch.top - y, true);
		...
		mRegions[MIDDLE_CENTER] = TextureRegionFactory.extractFromTexture(
				pTexture, pCenterPatch.left, pCenterPatch.top, pCenterPatch.width(), pCenterPatch.height(), true);

		mRegions[MIDDLE_RIGHT] = TextureRegionFactory.extractFromTexture(pTexture, pCenterPatch.left + pCenterPatch.width(),
